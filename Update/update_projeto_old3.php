...
if(empty($id)){
    ...
    $status_bet="n";
     ...
     $sql = "update loguser set status = :descricao where idbigbet = :bigId";

	$stmt = $conn->prepare($sql);                                  
	$stmt->bindParam(':status_loguser', $status_loguser, PDO::PARAM_STR); 
	$stmt->bindParam(':bigId', $id, PDO::PARAM_STR);   
	$stmt->execute(); 

    $sql = "update bets set statua = :status_bet where bigId = :bigId";

	$stmt = $conn->prepare($sql);                                  
	$stmt->bindParam(':status_bet',$status_bet, PDO::PARAM_STR); 
	$stmt->bindParam(':bigId', $id, PDO::PARAM_STR);   
	$stmt->execute(); 
